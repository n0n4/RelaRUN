...
namespace RelaRUN.Snapshots
{
    public class NetExecutorSnapper : INetExecutor
    {
        ...
        public void PreTick(float elapsedMS)
        {
            ...
            // debugging:
            if (elapsedMS >= TickMSTarget * 2)
            {
                Server.NetLogger.Log("Snapper Overticking --> " + elapsedMS + " / " + TickMSTarget);
            }
            ...
            {
                watch.Stop();
                if (watch.Elapsed.TotalMilliseconds > 10f)
                {
                    Server.NetLogger.Log("Snapper Tick took too long --> " + watch.Elapsed.TotalMilliseconds);
                }
            }
        }
